...
public class ProductDao {
    ...
    private static final String INSERT_PRODUCT_BY_PROVIDER = """
            INSERT INTO product (prod_id, shop_id, name, currency, image, images, price, min_price, max_price, price_before_discount,
             price_max_before_discount, price_min_before_discount, shop_location, description, stock, discount, provider_created_time, 
             status, provider, rating, like_count, attributes, skus)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            discount = VALUES(discount), provider_created_time = VALUES(provider_created_time), status = VALUES(status), provider = VALUES(provider), rating = VALUES(rating),
    ...
    public void add(ProductDto dto) {
        try {
            var param = new Object[]{
                    ...
                    dto.getCtime(), dto.getStatus(), dto.getProvider(), dto.getRating(), dto.getLikeCount(), dto.getAttributes(), dto.getSku()
            };
